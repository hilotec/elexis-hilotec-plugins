...
public class ArchivKGPrintView extends ViewPart {
	 ...
	 */
	private Object processKonsultation(Konsultation k, ITextPlugin tp,
			Object pos) {
		...
		pos = addParagraph("Diagnose", kd.getDiagnose(),
			kd.getDiagnoseICPC(), tp, pos);
		pos = addParagraph("Therapie", kd.getTherapie(), tp, pos);
		pos = addParagraph("Verlauf", kd.getVerlauf(), tp, pos);
		pos = addParagraph("Procedere", kd.getProzedere(),
			kd.getProzedereICPC(), tp, pos);
		pos = tp.insertText(pos, "\n", 0);

		return pos;
	}

	private Object addParagraph(String titel, String text, ITextPlugin tp,
			Object pos) {
		return addParagraph(titel, text, null, tp, pos);
	}

	private Object addParagraph(String titel, String text, String icpc,
			ITextPlugin tp, Object pos)
	{
	...
	public void doPrint(Konsultation kons, Kontakt adressat, boolean reversed) {
		brief = text.createFromTemplateName(kons, "ArchivKG", Brief.UNKNOWN,
				adressat, "Archiv-KG");
